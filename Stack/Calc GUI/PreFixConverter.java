...
{
	private String strToken;       //Token of preFix Data: digit, operator
	private Scanner stringScan;     //Makes tokens out of the lines postFix data
	private String postFix;
	private LinkedQueueClass tokens;
	private StringElement qStr;
	private String op;

	public String convert(String inpLine)
	{
        tokens = new LinkedQueueClass();
		stringScan = new Scanner(inpLine);

		strToken = stringScan.next();


			//Check if the String string starts with parenthesis
			if(strToken.charAt(0) == '(')
			{

			}

			//Check first token, must be digit
			else if(Character.isDigit(strToken.charAt(0)))
			{
				//Set the token as a stringElement
			    qStr = new StringElement(strToken);

			    //Add it to the queue
				tokens.addQueue(qStr);
			...
			//Else not a digit, return error
			else
			{
				postFix = "error";
		    }

			//Check next token, must be an op
			if(strToken.equals("+") ||
			   strToken.equals("-") ||
			   strToken.equals("*") ||
			   strToken.equals("/"))
    ...
    public String complexExpression(String expression)
    {
		Queue add;
		Queue sub;
		String num1;
		...
		//{
			...
			{
				//Set the token as a stringElement
			    //qStr = new StringElement(expression);
				...
				//Get next token
				expression = stringScan.next();
			}

		    //Check token, must be digit
